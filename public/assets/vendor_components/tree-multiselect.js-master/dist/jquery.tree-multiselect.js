...
function mergeDefaultOptions(options) {
  var defaults = {
    ...
    searchable: false,
    searchParams: ['value', 'text', 'description', 'section', 'isc'],
    sectionDelimiter: '/',
...
Tree.prototype.createAst = function (options) {
  ...
  options.each(function () {
    ...
    var item = Ast.createItem({
      ...
      isc: option.getAttribute('data-isc'),
...
Tree.prototype.createSearchBar = function (parentNode) {
  ...
  var searchNode = Util.dom.createNode('input', { class: 'search', placeholder: this.params.SearchPlacholderText });
...
Tree.prototype.render = function (noCallbacks) {
  ...
  for (var jj = 0; jj < this.keysToAdd.length; ++jj) {
    ...
    var selectedNode = Util.dom.createSelected(astItem, this.params.freeze, this.params.showSectionOnSelected, this.params.pluginType);
...
exports.createSelected = function (astItem, disableRemoval, showSectionOnSelected, pluginType) {
  ...
  if (showSectionOnSelected)  {
    if (pluginType == 'filter') {
      var thisInpName = 'defaultFilterOptions[]';
    } else {
      var thisInpName = 'defaultAddonOptions[]';
    }

    if (astItem.isc == 'true') {
      var DefaultChecked = exports.createNode('input', { type: 'checkbox', class: 'default-checked', name: thisInpName, 'data-key': astItem.id, 'value': astItem.value, 'checked':'true' });
    } else {
      var DefaultChecked = exports.createNode('input', { type: 'checkbox', class: 'default-checked', name: thisInpName, 'data-key': astItem.id, 'value': astItem.value });
    }
    
    node.insertBefore(DefaultChecked, node.firstChild);
  }
